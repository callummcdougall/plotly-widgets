...
def fetching_plotly_figures():
    ...
    d[9] = fig_9
    d[10] = px.colors.qualitative.swatches()
    ...
    d[13] = fig_13
    d[14] = px.colors.diverging.swatches_continuous()
    ...
    d[28] = fig_28
    df = px.data.stocks()
    d[29] = px.line(df, x='date', y="GOOG") # plotly express handles date formatting automatically (even if dates are strings)
    df = pd.read_csv('https://raw.githubusercontent.com/plotly/datasets/master/finance-charts-apple.csv')
    d[30] = go.Figure([go.Scatter(x=df['Date'], y=df['AAPL.High'])]) # same is true for plotly graph objects
    df = px.data.stocks(indexed=True)-1
    d[31] = px.area(df, facet_col="company", facet_col_wrap=3)
    df = pd.read_csv('https://raw.githubusercontent.com/plotly/datasets/master/finance-charts-apple.csv')
    ...
    d[34] = fig_34
    df = px.data.iris()
    d[35] = px.scatter(df, x="sepal_length", y="sepal_width", color="species", 
                    marginal_x="box", marginal_y="violin")
    df = px.data.tips()
    d[36] = px.scatter(df, x="total_bill", y="tip", color="sex", facet_col="day", marginal_x="box")
    return d
...
with st.expander(""):
    ...
    st.plotly_chart(d[23], config={'modeBarButtonsToAdd': [
        'drawline',
        'drawopenpath',
        'drawclosedpath',
        'drawcircle',
...
with st.expander(""):
    ...
    st.plotly_chart(d[27])
    st.code(code_28, language="python")
    st.plotly_chart(d[28])
